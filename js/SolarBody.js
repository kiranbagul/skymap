...
Planets.getGroundPosition = function (date, ra, dec, lat, lon) {
    ...
    Geometry.horizonCoordinates(date, ra, dec, lt, ln, h);
    return h;
};
...
Geometry.mod2pi = function (x) {
    var b = x / (2 * Math.PI);
    var a = (2 * Math.PI) * (b - Geometry.absFloor(b));
...
Geometry.horizonCoordinates = function (utc, ra, dec, lat, lon, h) {
